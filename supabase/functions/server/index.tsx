...
app.post("/make-server-09672449/messages/:userId", async (c) => {
  try {
    ...
    const { id, role, content, timestamp, feedback, attachments, isIncorrect, activityType, aiProvider } = await c.req.json();
    ...
    const message = { 
      ...
      content, 
      timestamp, 
      activityType: activityType || (role === 'user' ? 'user-activity' : 'ai-response'),
      aiProvider: aiProvider || (role === 'user' ? 'User' : 'Unknown'),
